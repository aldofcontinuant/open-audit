    ...
    public function graph()
    {
        ...
        switch ($report) {
           ...
           $sql = "SELECT DATE(change_log.timestamp) AS 'date', count(change_log.id) AS 'count' FROM change_log LEFT JOIN oa_group_sys ON (change_log.system_id = oa_group_sys.system_id) WHERE oa_group_sys.group_id = ? AND change_log.db_table = 'software' AND change_log.db_action = 'create' AND DATE(change_log.timestamp) >= ? AND DATE(change_log.timestamp) <= ? GROUP BY DATE(change_log.timestamp)";
            ...
            case "new_devices":
            $sql = "SELECT DATE(first_seen) AS 'date', COUNT(*) as count FROM system LEFT JOIN oa_group_sys ON (system.id = oa_group_sys.system_id) WHERE oa_group_sys.group_id = ? AND status = 'production' AND DATE(first_seen) >= ? AND DATE(first_seen) <= ? AND system.ip <> '' AND system.ip <> '0.0.0.0' AND system.ip <> '000.000.000.000' GROUP BY DATE(first_seen) ORDER BY DATE(first_seen) ";
            ...
            default:
            $sql = "SELECT DATE(first_seen) AS `date`, COUNT(*) as `count` FROM system LEFT JOIN oa_group_sys ON (system.id = oa_group_sys.system_id) WHERE oa_group_sys.group_id = ? AND status = 'production' AND DATE(first_seen) >= ? AND DATE(first_seen) <= ? AND system.ip <> '' AND system.ip <> '0.0.0.0' AND system.ip <> '000.000.000.000' ";
            $this->data['heading'] = "Devices Discovered 30";
            $data = array($group_id, $start_date, $end_date);
            $json = 'y';
            break;
